...
- Grid: N windows in area W x H. For each cols in 1...N: rows = ceil(N/cols), lastCount = N - (rows-1)*cols, score = max(|ln((W/cols)/(H/rows))|, |ln((W/lastCount)/(H/rows))|). Lowest score wins, ties to fewer rows. Full rows have `cols` cells, last row has `lastCount` cells stretched to `W / lastCount`. Gap 0.
  - On 16:9 and 16:10: 2 -> 2x1. 3 -> 3x1. 4 -> 2x2. 5 -> 3+2. 7 -> 4+3. 9 -> 5+4. 12 -> 4x3. Portrait 2 -> 1x2.
